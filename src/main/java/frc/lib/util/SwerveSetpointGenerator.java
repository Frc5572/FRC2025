...
public class SwerveSetpointGenerator {
    ...
    public SwerveSetpointGenerator(final SwerveDriveKinematics kinematics,
        final double maxDriveSpeed, final double maxModuleAcceleration,
     ...
     */
    public SwerveSetpoint getFeasibleSetpoint(final SwerveSetpoint previousSetpoint,
        final ChassisSpeeds desiredChassisSpeeds, final double allowedScrub) {
        ...
        SwerveSetpoint bestSetpoint = optimizeWithSlewRateLimit(
            getDesaturatedSwerveSetpoint(interpolateChassisSpeeds(previousSetpoint.chassisSpeeds,
                desiredSetpoint.chassisSpeeds, upperBound)).moduleStates,
            previousSetpoint.moduleStates, m_maxSpeedDiff, m_maxAngleDiff);
        ...
        for (int n = 0; n < 10; n++) {
            ...
            final SwerveSetpoint testSetpoint = optimizeWithSlewRateLimit(
                getDesaturatedSwerveSetpoint(
                    interpolateChassisSpeeds(previousSetpoint.chassisSpeeds,
                        desiredSetpoint.chassisSpeeds, alpha)).moduleStates,
                previousSetpoint.moduleStates, m_maxSpeedDiff, m_maxAngleDiff);
    ...
    private static ChassisSpeeds interpolateChassisSpeeds(final ChassisSpeeds a,
        final ChassisSpeeds b, final double alpha) {
        return new ChassisSpeeds((1 - alpha) * a.vxMetersPerSecond + alpha * b.vxMetersPerSecond,
            (1 - alpha) * a.vyMetersPerSecond + alpha * b.vyMetersPerSecond,
            (1 - alpha) * a.omegaRadiansPerSecond + alpha * b.omegaRadiansPerSecond);
    }

    private SwerveSetpoint optimizeWithSlewRateLimit(final SwerveModuleState[] targetStates,
        final SwerveModuleState[] currentStates, final double maxSpeedDiff,
    ...
    public double computeMaxModuleScrubs(final SwerveModuleState[] states) {
        ...
        for (int i = 0; i < states.length; i++) {
            final var cart1 =
                pol2cart(idealStates[i].speedMetersPerSecond, idealStates[i].angle.getRadians());
            final var cart2 =
     ...
     */
    public static SwerveModuleState optimizeModule(final SwerveModuleState desiredState,
        final SwerveModuleState currentState) {
        double targetAngle =
            placeInScope(desiredState.angle.getRadians(), currentState.angle.getRadians());
